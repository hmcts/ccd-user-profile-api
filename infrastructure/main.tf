...
locals {
  ...
  odlPreviewVaultName = "${var.product}-profile"
  oldNonPreviewVaultName = "${local.odlPreviewVaultName}-${var.env}"
  oldVaultName = "${(var.env == "preview" || var.env == "spreview") ? local.odlPreviewVaultName : local.oldNonPreviewVaultName}"
}
